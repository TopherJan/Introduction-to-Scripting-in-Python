...
def filter_by_year(statistics, year, yearid):
    ...
    """
    return list(filter(lambda statistics: int(statistics[yearid]) == year, statistics))

# print(filter_by_year(stats_list, 1871, 'yearID'))
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        new_list.append((row[info['playerid']], formula(info, row)))
    new_list.sort(key=lambda pair: pair[1], reverse=True)

    return new_list[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    master_stats = read_csv_as_list_dict(info['masterfile'], info['separator'], info['quote'])

    player_names = []

    for item in top_ids_and_stats:
        for row in master_stats:
            if row[info['playerid']] == item[0]:
                firstname = row[info['firstname']]
                lastname = row[info['lastname']]
                player_names.append("{:.3f} --- {} {}".format(item[1], firstname, lastname))
    return player_names
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    list_by_year = filter_by_year(batting_stats, year, info['yearid'])

    #get the top players of that year
    top_players = top_player_ids(info, list_by_year, formula, numplayers)

    #get the names of the top players
    player_names = lookup_player_names(info, top_players)
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    field_names = info['battingfields']

    indiv_stats = aggregate_by_player_id(batting_stats, info['playerid'], field_names)

    player_names = [name for name, stats in indiv_stats.items()]

    compounded_stats = []

    for player in player_names:
        compounded_stats.append((player, formula(info, indiv_stats[player])))

    compounded_stats.sort(key=lambda pair: pair[1], reverse=True)

    return lookup_player_names(info, compounded_stats[:numplayers])
